...
const defaultCachePlugins = ["babel", "commonjs", "node-resolve"];
...
export function addPluginCachingToConfig(
  ...
  const cachePluginsNames = [...defaultCachePlugins, ...cachePlugins];
  ...
  const cachingPlugins =
    ...
    buildConfig.plugins?.map((plugin) => {
      ...
      const config = cachePluginsNames.includes(plugin.name) ? {} : null;
      if (!config) {
        return plugin;
      }
